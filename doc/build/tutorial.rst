...
To round out the WHERE / ORDER BY clauses, the HAVING / GROUP BY clauses
serve the same purpose at the aggregate function level and are available
via the :meth:`_sql.Select.having` and :meth:`_sql.Select.group_by`
methods.  These functions are normally used with SQL aggregate functions,
which in SQLAlchemy are available using a namespace called :data:`_sql.func`::
    ...
    ... )
    SELECT user_account.name, count(address.id) AS count_1
